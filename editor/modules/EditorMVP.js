...
import { onChangeBgColor }  from "./bgColorEditor.js";  // Nuevo módulo
...
export function EditorMVP({ htmlUrl, uid }) {
  ...
  // 3) Render + aplicar ediciones + marcar tipos + iconos
  useEffect(() => {
    if (!html) return;
    const root = containerRef.current;
    root.innerHTML = html;

    // Aplica las ediciones guardadas
  ...
  // 4) Click global para menú contextual (texto / link / fondo / color)
  useEffect(() => {
    const handler = e => {
    ...
    ctxMenu.show && ReactDOM.createPortal(
      React.createElement(
        ...
        ctxMenu.types.includes("text") &&
          React.createElement("button", { onClick: () => onChangeRichText(ctxMenu, uid, pageId, hideMenu) }, "Editar texto"),
        ctxMenu.types.includes("link") &&
          React.createElement("button", { onClick: () => onChangeLink(ctxMenu, uid, pageId, hideMenu) }, "Editar link"),
        ctxMenu.types.includes("bgImage") &&
          React.createElement("button", { onClick: () => onChangeBgImage(ctxMenu, uid, pageId, hideMenu) }, "Editar fondo"),
        ctxMenu.types.includes("bgColor") &&
          React.createElement("button", { onClick: () => onChangeBgColor(ctxMenu, uid, pageId, hideMenu) }, "Editar color")
      ),
      document.body
    )
  );
